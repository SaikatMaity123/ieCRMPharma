...
const LeaveStatus = ({navigation}) => {
    ...
    try {
      AsyncStorage.getItem('UserData').then(value => {
        if (value != null) {
          ...
          setIDEmployee(user.Empno);
          settenantId(user.HRMSLeaveKey);
          fetchLeaveApplications(user.Empno,user.HRMSLeaveKey);
        }
      });
    } catch (error) {
      Alert.alert(error);
  ...
  const fetchLeaveApplications = async (empNo,tenantId) => {
    setLoading(true);

    const currentYear = moment().format('YYYY');

    const apiUrl =
      `${Hrms_URL}LeaveApplicationHistory` +
      `?tenantId=${tenantId}` +
      `&empNo=${empNo}` +
      `&year=${currentYear}`;

    console.log('Leave History URL:', apiUrl);
    ...
    try {
      ...
      const cleanedData = data.map(item => ({
        ...
        ApplicationDate:
          typeof item.ApplicationDate === 'object'
            ? '-'
            : item.ApplicationDate ?? '-',

        FromDate:
          typeof item.FromDate === 'object' ? '-' : item.FromDate ?? '-',

        ToDate: typeof item.ToDate === 'object' ? '-' : item.ToDate ?? '-',
        ...
        TotalDaysApplied:
          item.TotalDaysApplied !== undefined ? item.TotalDaysApplied : '-',
      ...
      Alert.alert('Error', 'Failed to fetch leave application history.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSearch = text => {
    ...
    const filteredData = leaveApplications.filter(
      item =>
        String(item.ApplicationID).toLowerCase().includes(searchText) ||
        String(item.LeaveType).toLowerCase().includes(searchText) ||
        String(item.LeaveDuration).toLowerCase().includes(searchText) ||
        String(item.ApplicationDate).toLowerCase().includes(searchText) ||
        String(item.FromDate).toLowerCase().includes(searchText) ||
        String(item.ToDate).toLowerCase().includes(searchText) ||
        String(item.ApplicationStatus).toLowerCase().includes(searchText) ||
        String(item.TotalDaysApplied).toLowerCase().includes(searchText) ||
        String(item.Reason).toLowerCase().includes(searchText) ||
    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <TouchableOpacity
          ...
          onPress={() => fetchLeaveApplications(useIDEmployee,tenantId)}>
        ...
        <FlatList
          ...
          renderItem={({item}) => (
            <View style={styles.card}>
              ...
              {/* Leave Duration */}
              <Text style={styles.dateText}>
                {item.FromDate} - {item.ToDate} ({item.LeaveDuration})
              </Text>

              {/* Days */}
              <Text style={styles.appIdText}>
                Total Days : {item.TotalDaysApplied}
              </Text>
              ...
              {item.Remarks !== '-' && item.Remarks !== '' && (
                <Text style={styles.reasonText}>Remarks : {item.Remarks}</Text>
              )}
...
const styles = StyleSheet.create({
  ...
  pendingBadge: {
    ...
    backgroundColor: '#dc2626', // 🟣 Purple (fallback for unknown)
  },
