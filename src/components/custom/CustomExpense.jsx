...
const CustomExpense = props => {
    ...
    try {
      AsyncStorage.getItem('UserData').then(value => {
        if (value != null) {
          ...
          //console.warn(`${url}${cleanedPathProfile}`);
        }
      });
    } catch (error) {
      console.log(error);
  ...
  const captureAndDownload = async () => {
    const imageUrl = 'https://coreapitest.hr.mendine.co.in/Images/';
    ...
    const filePath = `${RNFS.PicturesDirectoryPath}/${fileName}`; // Save to Pictures folder
    ...
    <View style={{flex: 1}}>
      <DrawerContentScrollView
        ...
        {/* <ImageBackground
          ...
          style={{padding: 20}}> */}
          <View style={{backgroundColor: '#005696', padding: 20}}>
          {/* <Image
